...
public class GameJDialog extends JDialog {
	...
	public GameJDialog(Frame owner) {
		...
		JLabel label = new JLabel("Ingrese nombre o ID del videojuego:");
		...
		String[] cols = {"ID", "Título", "Plataforma"};
		...
		table.addMouseListener(new MouseAdapter(){
			public void mouseClicked(MouseEvent e){
				if (e.getClickCount()==2 && table.getSelectedRow()>=0){
					...
					selectedGame = allGames.get(mrow);
					dispose();
				}
			}
		});
	...
	private void loadAllGames() {
		try {
			allGames = new GamesController().get();
			for (Game c : allGames) {
